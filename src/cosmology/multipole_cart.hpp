...
struct multipole_index
  {
    ...
    static const vector<multipole_index> indices;
    ...
    {
      using ret_t = decltype(func(multipole_index<K>::indices[0], multipole_index<order-K>::indices[0]));
    ...
    friend ostream& operator<<(ostream& s, const multipole_index& i)
    {
...
multipole<T, TPREV...> : public multipole<TPREV...>
{
  ...
  using index_t = multipole_index<order>;
  static const vector<index_t>& indices() { return index_t::indices;}
  ...
  {
    return A[index_t::indexmap[get_index(indices)]];
  }
...
public:
	...
	if constexpr (order >= 1)
	  {
	    ...
	    (*this)[i] +=
	      static_cast<T>(factor) * i.template cycle<MP::order>([&](auto q, auto w) { return static_cast<T>(w.prod(a)) * static_cast<T>(M[q]); });
	  }
	...
	if constexpr (order >= 2)
	  {
		...
		(*this)[i] +=
		  + static_cast<T>(static_cast<shift_t>(factortab[order]) * a.squaredNorm() * i.template cycle<order-2>([&](auto q, auto w){return q.prod(a) * (w.i[0]==w.i[1]);})) * static_cast<T>(M.coeff({})) ;
	      }
		...
		(*this)[i] -=
		  static_cast<T>(static_cast<shift_t>(factor) * i.template cycle<order-2>([&](auto q, auto w){return q.prod(a) * (w.i[0]==w.i[1]);})) * get_t<1>::indexsum([&](auto j) { return static_cast<T>(M[j]) * static_cast<T>(j.prod(a)); }) ;
	      }
	    if constexpr(MP::order == 2)
	      {
		const double factortab[] = {0,0, 0, 2.0/3, 5.0/6};
		(*this)[i] += 
		  static_cast<T>(-factortab[order]) * get_t<1>::indexsum([&](auto j) {return static_cast<T>(j.prod(a)) * i.template cycle<1>([&](auto q, auto w) {return static_cast<T>(M.coeff(j, q)) * static_cast<T>(w.template cycle<2>([&](auto wa, auto wb){ return wb.prod(a) * (wa.i[0] == wa.i[1]);}));});});
	...
	if constexpr (order == 4)
	  {
	    if constexpr(MP::order == 0)
	      {
		(*this)[i] +=
		  + static_cast<T>(static_cast<shift_t>(1.0 / 16 * i.template cycle<2>([&](auto q, auto w) -> double {return (q.i[0] == q.i[1] && w.i[0] == w.i[1]);})) * pow2(a.squaredNorm())) * static_cast<T>(M.coeff({}));
	      }
	    if constexpr(MP::order == 1)
	      {
		(*this)[i] +=
		  - static_cast<T>(static_cast<shift_t>(5.0 / 8) * a.squaredNorm()) * i.template cycle<1>([&](auto q, auto w) {return static_cast<T>(M(q)) * static_cast<T>(w.template cycle<1>([&](auto wa, auto wb){return a[wa.i[0]] * (wb.i[0] == wb.i[1]);}));})
		  + static_cast<T>(static_cast<shift_t>(1.0/4 * i.template cycle<2>([&](auto q, auto w) -> double{ return (q.i[0] == q.i[1] && w.i[0] == w.i[1]); })) * a.squaredNorm()) * get_t<1>::indexsum([&](auto j) { return static_cast<T>(M[j]) * static_cast<T>(j.prod(a)); });
	      }
	    if constexpr(MP::order == 2)
	      {
		(*this)[i] +=
		  - static_cast<T>(static_cast<shift_t>(5.0 / 12) * a.squaredNorm()) * i.template cycle<2>([&](auto q, auto w) {return static_cast<T>(M(q)) * ((w.i[0] == w.i[1]));})
		  + static_cast<T>(1.0 / 6 * i.template cycle<2>([&](auto q, auto w) {return static_cast<double>(q.i[0] == q.i[1] && w.i[0] == w.i[1]);})) * get_t<2>::indexsum([&](auto j) {return static_cast<T>(M.coeff(j)) * static_cast<T>(j.prod(a));});
	      }
	    if constexpr(MP::order == 3)
	      {
		(*this)[i] +=
		  - static_cast<T>(1.0 / 2) * get_t<1>::indexsum([&](auto j) {return static_cast<T>(j.prod(a)) * i.template cycle<2>([&](auto q, auto w) {return static_cast<T>(M.coeff(j, q)) * (w.i[0] == w.i[1]);});});
	      }
	  }
      }
    if constexpr (MP::order > 0)
  ...
  void makelocal_add_contrib(Vector<shift_t, 3> a, const MP& M, auto* sum_f32, auto* sum_bf16) const
  {
    if constexpr(order + MP::order <= MAX::order)
      {
	//using Tuse = typename MAX::get_t<order + MP::order>::value_type ;
	//using Tuse = typename MAX::get_t<max(order, MP::order)>::value_type ;
	using Tuse = typename MP::value_type;
	...
	for (auto& i : indices())
	  {
	    ...
	    contrib +=
	      static_cast<Tuse>(factorial2(2*(MP::order + order)-1) / factorial2(static_cast<Tdouble>(2*MP::order-1)) * pow(-1, order) / factorial(order)
				* inva * i.prod(e)) * M.indexsum([&](auto j){return static_cast<Tuse>(M[j]) * static_cast<Tuse>(j.prod(e));});
		...
		if constexpr (MP::order == 2)
		  {
		    contrib += static_cast<Tuse>(pow5(inva)) * static_cast<Tuse>(M[i]);
		  }
	      }
	    if constexpr (order >= 2)
	      {
		...
		contrib += static_cast<Tuse>(factor * pow3(inva) * i.template cycle<order-2>([&](auto q, auto w) { return q.prod(e) * (w.i[0] == w.i[1]); })) * M.indexsum([&](auto j) { return static_cast<Tuse>(M[j]) * static_cast<Tuse>(j.prod(e)); });
	      }
	    if constexpr (order == 3)
	      {
		if constexpr (MP::order == 1)
	    ...
	    if constexpr(is_same_v<Tuse, gpu_bfloat16>)
	      {
		(*sum_bf16).template get<order>()[i] += contrib;
	      }
	    else
	      {
		(*sum_f32).template get<order>()[i] += contrib;
	      }
	  }
      }
    if constexpr (MP::order > 0)
  ...
  {
    typename get_t<0>::value_type contrib = 0;
    for (auto& i : indices())
      {
  ...
  {
    Vector<typename get_t<0>::value_type, 3> F = {0,0,0};
  ...
  template<typename U, typename... UPREV>
  explicit multipole(const multipole<U, UPREV...>& b) :
    prev_t(static_cast<multipole<TPREV...>>(b.prev())),
    A(b.A.template cast<T>())
  {
  }
...
template <unsigned order>
array<Tint, pow<order, 1>(3)> multipole_index<order>::indexmap;

GOOPAX_PREPARE_STRUCT(multipole)

template <unsigned order>
const vector<multipole_index<order>> multipole_index<order>::indices = []()
 {
   vector<multipole_index<order>> ret((order + 1) * (order + 2) / 2);

  vector<multipole_index> sum;
